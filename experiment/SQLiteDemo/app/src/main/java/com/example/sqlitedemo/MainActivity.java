...
public class MainActivity extends AppCompatActivity {
    ...
    public void onConfigurationChanged(Configuration configuration){
        ...
        if(configuration.orientation==Configuration.ORIENTATION_LANDSCAPE){
            ...
            View view=getLayoutInflater().inflate(R.layout.left_fragment,null);
            ListView wordAndId_listview=(ListView) view.findViewById(R.id.left_listview);
            ArrayList<Map<String,String>> wordAndId_list=getAllIdAndWord();
//            setWordAndIdAdapter(wordAndId_listview,wordAndId_list);

            SimpleAdapter simple = new SimpleAdapter(MainActivity.this, wordAndId_list, R.layout.word_and_id_layout,
                    new String[]{Words.Word._ID, Words.Word.TABLE_COLUMN_WORD},
                    new int[]{R.id.word_id, R.id.word});

            Log.i("tag","simple:"+simple);

            wordAndId_listview.setAdapter(simple);

            View rightView=getLayoutInflater().inflate(R.layout.right_fragment,null);

            ListView meaningAndSample_listview=(ListView) rightView.findViewById(R.id.right_listview);

            ArrayList<Map<String,String>> meaningAndSample_list=getAllMeaningAndSample();

            Log.i("tag",meaningAndSample_list.get(0).get(Words.Word.TABLE_COLUMN_MEANING));


            setMeaningAndSampleAdapter(meaningAndSample_listview,meaningAndSample_list);
        }else {
    ...
    //从数据库中读入内容
    public ArrayList<Map<String, String>> getAllIdAndWord() {
        ArrayList<Map<String, String>> list = new ArrayList<>();
    ...
    //从数据库中读入内容
    public ArrayList<Map<String, String>> getAllMeaningAndSample() {
        ArrayList<Map<String, String>> list = new ArrayList<>();
    ...
    //增加适配器，在列表中显示单词
    public void setWordAndIdAdapter(ListView wordAndIdListview, ArrayList<Map<String, String>> item) {
        SimpleAdapter simple = new SimpleAdapter(MainActivity.this, item, R.layout.word_and_id_layout,
                new String[]{Words.Word._ID, Words.Word.TABLE_COLUMN_WORD},
                new int[]{R.id.word_id, R.id.word});

        wordAndIdListview.setAdapter(simple);
    }

    //增加适配器，在列表中显示单词
    public void setMeaningAndSampleAdapter(ListView wordAndIdListview, ArrayList<Map<String, String>> item) {
        SimpleAdapter simple = new SimpleAdapter(MainActivity.this, item, R.layout.meaning_and_sample_layout,
                new String[]{Words.Word.TABLE_COLUMN_MEANING, Words.Word.TABLE_COLUMN_SAMPLE},
                new int[]{R.id.meaning, R.id.sample});
